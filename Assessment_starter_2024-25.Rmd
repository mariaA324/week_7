...
most_prescribed <- data_march2024 %>% 
  ...
  summarise(sum = sum(paid_quantity)) %>% 
  arrange(desc(sum)) %>% 
  head(most_prescribed, n = 10)
```
...
joined <- full_join(data_march2024, health_boards, by = join_by(hbt == hb))
...
```{r}
joined_2 <- full_join(population_data, joined)
```
...
```{r}
paracetamol_plot <- joined_2 %>% 
  filter(paid_date_month == "202403") %>% 
  filter(str_detect(bnf_item_description, "PARACET.*CAP")) %>% 
  group_by(hb_name) %>% 
  summarise(sum = sum(number_of_paid_items),per_person = (sum/mean(hb_population)))

paracetamol_plot_2 <- paracetamol_plot %>% 
  ggplot(aes(x =reorder(hb_name, per_person), y = per_person, fill = hb_name))+
  geom_col()+
  coord_flip() +
  labs(title = "Paracetamol Capsules/Caplets  ordered per person in March 2024 across Scottish Health Boards")
...
```{r}
covid <- read.csv("https://www.opendata.nhs.scot/dataset/84393984-14e9-4b0d-a797-b288db64d088/resource/64131d22-ab47-43ff-9674-ebe8ed7edbd7/download/pitc202006.csv") %>% 
  clean_names()

joined_3 <- full_join(covid, health_boards, by = join_by(hbt == hb))

joined_3_plot <- full_join(joined_3, population_data)
...
plot2_plotted <-  plot2 %>% 
  ...
  geom_col()+
  coord_flip() +
  labs(title = "Paracetamol Capsules/Caplets  ordered per person in June 2020 across Scottish Health Boards")
library(patchwork)

combined <- paracetamol_plot_2 + plot2_plotted
...
```{r}
consecutive_data_1 <- full_join(consecutive_data, health_boards, by = join_by(HBT == hb))

conseq_join <- full_join(consecutive_data_1, population_data)

conseq_join <- conseq_join %>% 
  filter(str_detect(BNFItemDescription, "PARACET.*CAP")) %>% 
   group_by(PaidDateMonth) %>% 
  summarise(sum = sum(NumberOfPaidItems),per_person = (sum/mean(hb_population)))

conseq_join %>% 
  ggplot(aes(x = PaidDateMonth, y = per_person))+
  geom_line()
